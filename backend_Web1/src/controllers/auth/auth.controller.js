...
exports.login = async (req, res) => {
  try {
    ...
    // FIX: pehle "role" (jo frontend pe user select karta hai — Client/
    // GD/SMM dropdown) kabhi check hi nahi hota tha. Isliye galat role
    // select karke bhi login "success" ho jaata tha, aur "Access denied"
    // sirf BAAD me, dashboard khulte hi (pehli protected API call pe)
    // aata tha — jo confusing UX tha.
    // Ab agar frontend "role" (ya "loginAs"/"selectedRole") bhejta hai,
    // to usko turant, LOGIN ke waqt hi, DB ke actual role se match
    // karte hain — mismatch hote hi yahin turant block kar dete hain.
    const selectedRole = req.body.role || req.body.loginAs || req.body.selectedRole;
    ...
    // NEW: role mismatch ab yahin pakda jaata hai, dashboard tak
    // jaane hi nahi dete. Case-insensitive comparison (role.middleware.js
    // jaisa hi pattern) taaki "Client" vs "client" jaisi dikkat na ho.
    if (selectedRole && String(selectedRole).toLowerCase() !== String(user.role).toLowerCase()) {
      return res.status(403).json({
        success: false,
        msg: `You are not registered as a ${selectedRole}. Please select the correct login type.`
      });
    }
